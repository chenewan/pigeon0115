...
public class MybatisPlusConfig {
    ...
    //定义db4
    @Bean(name = "db4")
    @ConfigurationProperties(prefix = "spring.datasource.db4")//与配置文件中的层次结构相同
    public DataSource db4() {
        return DruidDataSourceBuilder.create().build();
    }
    //定义db5
    @Bean(name = "db5")
    @ConfigurationProperties(prefix = "spring.datasource.db5")//与配置文件中的层次结构相同
    ...
    public DataSource multipleDataSource(@Qualifier("db1") DataSource db1, @Qualifier("db2") DataSource db2,
                                         @Qualifier("db3") DataSource db3, @Qualifier("db4") DataSource db4,@Qualifier("db5") DataSource db5) {
        DynamicDataSource dynamicDataSource = new DynamicDataSource();
        Map<Object, Object> targetDataSources = new HashMap<>();
        targetDataSources.put(DBTypeEnum.db1.getValue(), db1);
        targetDataSources.put(DBTypeEnum.db2.getValue(), db2);
        targetDataSources.put(DBTypeEnum.db3.getValue(), db3);
        targetDataSources.put(DBTypeEnum.db4.getValue(), db4);
        targetDataSources.put(DBTypeEnum.db5.getValue(), db5);
    ...
    public SqlSessionFactory sqlSessionFactory() throws Exception {
        ...
        sqlSessionFactory.setTypeAliasesPackage("com.byd.emg.pojo");
